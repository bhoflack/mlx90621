...
function readCompensationPixel() {
    ...
    return 256 * response[1] + response[0]
}

function readAll() {
    serial.writeLine("readAllPixels: creating command");

    const f = pins.createBufferFromArray([0x2, 0x0, 0x0, 0x1])
    serial.writeLine("write ptat command: " + f.toHex())
    pins.i2cWriteBuffer(0x60, f, true);

    const e = pins.createBufferFromArray([0x2, 0, 1, 0x40])
    serial.writeLine("readAllPixels: writing command");
    pins.i2cWriteBuffer(0x60, e, true);
    serial.writeLine("readAllPixels: reading pixels");
